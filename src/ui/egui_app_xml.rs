...
impl InstallerApp {
    ...
    fn get_page_layout(&mut self, page: WizardPage) -> Option<&LayoutTree> {
        ...
        eprintln!("→ 尝试加载布局: {}", layout_file);
        
        // 优先从嵌入资源加载
        let layout_content = if crate::resources::has_embedded_resources() {
            ...
            RuntimeResources::get_layout(&selected_file)
                .or_else(|_| RuntimeResources::get_layout(&layout_file))
                .ok()?
        } else {
            // 开发模式：从文件系统加载
            let layout_path = self.config_base_path.join(&layout_file);
    ...
    fn load_texture(&self, ctx: &egui::Context, relative_path: &str, name: &str) -> Option<egui::TextureHandle> {
        ...
        // 优先从嵌入资源加载
        if crate::resources::has_embedded_resources() {
            // 提取文件名
            ...
            return self.load_texture_from_bytes(ctx, &image_data, name);
        }
        
        // 开发模式：从文件系统加载
        let full_path = self.config_base_path.join(relative_path);
        
        // 检查是否有 DPI 适配的版本
        let dpi_path = if self.dpi_config.scale_factor >= 1.5 {
            let stem = full_path.file_stem()?.to_str()?;
            let ext = full_path.extension()?.to_str()?;
            let parent = full_path.parent()?;
            parent.join(format!("{}@2x.{}", stem, ext))
        } else {
            full_path.clone()
        };
        
        // 优先尝试 DPI 适配版本
        let path_to_load = if dpi_path.exists() {
            dpi_path
        } else {
            full_path
        };
        ...
        match image::open(&path_to_load) {
            ...
            Err(e) => {
                tracing::warn!("Failed to load texture {:?}: {}", path_to_load, e);
                None
            }
        }
    }
