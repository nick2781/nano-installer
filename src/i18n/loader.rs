...
impl LanguageLoader {
    ...
    fn load_embedded_pack(&self, locale: &str) -> Result<Vec<u8>> {
        ...
        // 语言包文件未找到
        // nano-installer 是一个工具，不嵌入语言包
        // 语言包应该从用户项目（如 examples/TapTap/locales）动态加载
        tracing::error!("Language pack file not found for locale: {}", locale);
        tracing::error!("Searched paths:");
