...
文件的目录，变成一个 Windows 安装程序，图标、界面和文案都用你自己的。不需要宿主的安装框架，用户
机器上也不需要预装任何东西。
...
- 安装目录可以用 Windows 标准的文件夹选择框来选，也可以手动改：鼠标拖选或双击选词、复制粘贴、
  写错了撤销一步。
- 输入中日韩文字时使用正规的输入法组合窗，候选框会跟在光标下方。
- 项目在配置里打开 `install.require_admin` 后，安装包会自己向 Windows 申请管理员权限，
  不再需要用户右键「以管理员身份运行」。
- 关闭窗口前会先问一句本地化确认问题。
- 卸载完成后立刻回收安装目录；用户自己往里面放过文件时，目录会保留下来。
