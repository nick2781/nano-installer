...
安装会暂存 payload、拒绝覆盖已存在目标、在 manifest 中记录已部署文件，并注册卸载器。移除只删除
manifest 跟踪的文件。

仍未实现：升级、完整取消与恢复、任务进度与页面切换、Rhai 脚本执行、快捷方式与开机启动、自动 UAC、
代码签名，以及 Win7 虚拟机验收。runtime 启动时会把整个内嵌 bundle 读入内存，生产使用前需要改为偏移
访问或内存映射。
