...
pub fn replace_exe_icon(exe_path: &Path, icon_path: &Path) -> Result<()> {
    ...
    // 读取图标文件
    let icon_data = std::fs::read(icon_path).context("Failed to read icon file")?;
    ...
    unsafe {
        ...
        for (idx, entry) in icon_dir.entries.iter().enumerate() {
            ...
            // 更新单个图标资源 (RT_ICON)
            let icon_bytes =
                &icon_data[entry.offset as usize..(entry.offset + entry.size) as usize];
            UpdateResourceW(
                handle,
                RT_ICON,
        ...
        // 提交更改
        EndUpdateResourceW(handle, false).context("Failed to end update resource")?;
    }
