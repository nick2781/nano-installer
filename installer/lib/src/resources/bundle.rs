...
impl ResourceBundle {
    ...
    pub fn unpack(data: &[u8]) -> Result<Self> {
        ...
        if version != VERSION && version != 1 {  // 兼容旧版本
    ...
    pub fn get_segment(&self, segment_type: SegmentType) -> Option<&ResourceSegment> {
        self.segments.iter().find(|s| s.segment_type == segment_type)
    }
    ...
    /// 收集目录中的所有文件
    fn collect_files(dir: &Path, base_dir: &Path, prefix: &str, files: &mut Vec<(String, Vec<u8>)>) -> Result<()> {
        for entry in fs::read_dir(dir)? {
            ...
            if path.is_file() {
                ...
                let name = format!("{}/{}", prefix, relative_path.to_string_lossy().replace('\\', "/"));
