...
pub fn register(engine: &mut Engine, ctx: ScriptContext) {
    ...
    engine.register_fn(
        ...
        move |name: &str, exe_path: &str| -> bool {
            #[cfg(windows)]
            {
                if let Ok(userprofile) = std::env::var("USERPROFILE") {
                    let desktop = PathBuf::from(userprofile).join("Desktop");
                    let shortcut = desktop.join(format!("{}.lnk", name));
                    let created = create_lnk(&shortcut, exe_path);
                    if created {
                        c.record_shortcut(shortcut);
                    }
                    return created;
                }
                false
    ...
    engine.register_fn(
        ...
        move |name: &str, exe_path: &str, folder: &str| -> bool {
            #[cfg(windows)]
            {
                if let Ok(appdata) = std::env::var("APPDATA") {
                    ...
                    let shortcut = folder_path.join(format!("{}.lnk", name));
                    let created = create_lnk(&shortcut, exe_path);
                    if created {
                        c.record_directory(folder_path);
                        c.record_shortcut(shortcut);
                    }
                    return created;
                }
    ...
    engine.register_fn("delete_desktop_shortcut", |name: &str| -> bool {
        ...
        {
            if let Ok(userprofile) = std::env::var("USERPROFILE") {
                let shortcut = PathBuf::from(userprofile)
                    .join("Desktop")
        ...
        .output()
    {
        Ok(output) => {
            if output.status.success() {
                ...
                tracing::error!(
                    "[script] Shortcut creation failed: {}",
