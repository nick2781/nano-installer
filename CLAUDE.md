...
14. **Stub 必须重编译** — 修改 Rust 代码后必须 `touch` 源文件再 `cargo build --release --bin lzma-x64`（确认有 `Compiling` 输出），然后重新 build installer。仅修改 XML/locale/config 不需要重编译 stub，但需要重新 build installer
