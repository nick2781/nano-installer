...
- If you change installer runtime or stub source code under `installer/`, you must rebuild the related stubs before rebuilding any setup package.
  - At minimum this means rebuilding `nano-installer-lzma`, `nano-installer-zlib`, `uninst`, and `nano-installer-cli` in `--release` when the packaged setup depends on them.
