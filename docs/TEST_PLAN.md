...
与用户数据清理规则。布局测试覆盖嵌套流式容器与间距、百分比尺寸、进度条的裁剪、
越界页面的回退，以及进度页与状态文案绑定。
...
升级、失败回滚、快捷方式、自启动、数据保留选项、页面切换与进度显示已实现，但尚未在真实
Win7 VM 中验收。

项目 Rhai 脚本已接入，覆盖用例见 `crates/nano-installer-core/src/script/mod.rs` 的单元测试：
脚本部署文件与清单、脚本失败回滚、`run_tracked_uninstall` 重放清单，以及脚本漏调时的库回退。
