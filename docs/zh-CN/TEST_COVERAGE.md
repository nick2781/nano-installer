...
`\scripts\run_tests.ps1` 会跑 368 条用例，一条命令一个 target：核心库 264 条，真构建并运行安装包的
...
| 安装包级 | 65 | 构建好的安装包在这台机器上装了一遍，它的窗口也是真的被驱动起来的：payload 字节（被改动过的安装包会连目标目录都不建就拒绝这次安装）、manifest、卸载项自己那组字段、快捷方式、自启动、项目脚本、脚本要跑的辅助程序、向导窗口、页面钩子把向导跳过一页并按来路退回、它自己页面上那些要等点击的行为、在列表上滚动滚轮、脚本的提示与提问所画的那张由点击作答的卡片、页面上的取值交到脚本手里、勾选决定这次装哪些组件、脚本按类型写下的每个注册表值以及视图名选中的是哪一份拷贝、
