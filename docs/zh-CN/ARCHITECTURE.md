...
`VBox`/`HBox`/`Content` 流式布局（含内边距、外边距与百分比尺寸），进度条，复选框与可展开面板
交互，静态链接渲染，运行时切换语言，无边框圆角窗口，任务栏图标，双缓冲绘制，拖动、最小化和关闭。
...
链接输入（当前渲染出的链接还不可点击，目录选择框也未接入），以及在真实 Windows 7 SP1 机器上的
生产验证。
