...
脚本会构建构建器、两个运行时和卸载运行时，再用真实的 ZIP 与 7z 归档验证解包是否正确，
...
示例项目生成 `examples/TapTap/dist/TapTap_Setup.exe`。它的应用文件（payload）
`examples/TapTap/payload/app.7z` 没有存进仓库，构建之前你要自己放一个 7z 归档进去。
...
默认又装到 `Program Files` 下，所以双击之后 Windows 会先弹一个提权确认框。你确认之后，
安装包就带着需要的权限运行。
