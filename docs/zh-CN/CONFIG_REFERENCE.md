...
- `{ "file": "%ProgramFiles(x86)%\\...\\msedgewebview2.exe" }`：这个文件在就算有。路径里的环境
  变量会先展开。
- `{ "registry": { ... } }`：读注册表。`key` 要打开的键，`HKCU` 或 `HKLM` 开头。键名还可以带上视图：
  `HKLM32` 是 `HKLM` 里 32 位程序看到的那份，`HKLM64` 是 64 位程序读到的那份，按位宽分开装的运行库
  就靠它来问。`name` 要读的值，不写就只问这个键在不在；写了 `name` 还能比一次，`equals` 是逐字相等，
  `at_least` 按点分数字比较。
