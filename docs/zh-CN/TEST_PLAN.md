...
资源读回来，核对权限级别与 DPI 行为是否与项目配置推出来的结果一致。有一项测试会写 `HKCU`，在受限
环境中默认忽略，要跑到它得在隔离虚拟机里显式执行。
...
卸载按钮，然后检查装好的文件、用户自建的文件有没有保留、失败回滚，以及卸载完成后安装目录是不是
马上消失、临时目录里的清理副本有没有退出。
...
升级、失败回滚、快捷方式、自启动、数据保留选项、页面切换与进度显示都已经实现，只是还没在真实
Windows 7 虚拟机上验收。
