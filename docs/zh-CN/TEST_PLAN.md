...
每条用例一行，以及带结果着色的完整输出。用例那一行写的是它守什么：中文页取自[用例说明](TEST_CASES.md)，
那份表里没有的用例退回源码里它上方的文档注释、再退回用例名，另外还有[测试覆盖](TEST_COVERAGE.md)
里说它失败就会坏掉的行为和配置项。
页面把同一次运行的两个读法绑在一起：目标那一行链到在它里面跑过的用例，末尾的说明给出用例行与
结果行一致的计数。页面还有一节「逐条行为」，把覆盖文档里逐条写明的每条行为都列出来，并给出这一轮为它跑到的
用例、这些用例各自的结果和它们跑在哪一层（核心库是进程内驱动库代码的用例，安装包级是真的构建并
安装了一遍安装包）：文档给某条行为写了用例、这一轮一条也没跑到的就写「未跑到」，文档自己承认
没人守着的部分也排在那一节里，没测到的行为不会从页面上消失。跑过 `scripts/capture_setup_snapshots.ps1` 之后，页面还会贴出安装包真实页面的
截图，按工程声明的页面分节，一节一页，并列出抓图时测到的每一项，写明页面尺寸、语言、缩放和它来自哪个
版面文件。抓图要有桌面会话，构建机上没有就不贴。CI 作业把这两个文件都作为
...
页面钩子则覆盖它按 `from` 挑页、它读到的向导与机器、它返回非文本时的报错，以及脚本里没有这个函数时
页序不变。
...
- 安装包内容：运行时读取的尾部标记、贴在真实 PE 之后的 payload、版本与清单资源，payload
  格式对应的运行时，以及工程自己的命令在成品上的那一步：它跑过之后安装包照常安装，它拒绝时构建
  中止且不留安装包。
- 安装：payload 落到磁盘且逐字节一致、manifest 记录实际写入的内容、卸载项指向部署出来的卸载程序、
  带上容量与 Windows 安装列表要的静默卸载命令，并声明这里没有修改和修复步骤、
  配置的 `%TEMP%` 路径会被展开、`--dir` 优先于配置路径，未声明支持或参数写错的运行不落任何产品文件；
  一次运行写下的日志（默认在临时目录，或 `--log` 指定的文件）如实记下产品、机器与每一步，失败时它
  就是交给支持人员的那份文件。
...
- 键盘：Tab 按页面声明控件的顺序逐个走到它们身上，焦点环画在键盘当前所在的那个控件上；空格与回车做那个
  控件本身会做的事（这里的复选框翻面、按钮走进下一页），走到最后一个再按 Tab 回到第一个，一页上什么都没
  声明时 Tab 无事可做。只有从前往后这一半写在这条里：窗口用例投递的按键消息不带修饰键，Shift 由运行时
  自己向 Windows 问，所以往回走由单元用例守住。
- 高对比：开关打开时，页面、卡片、卡片边与焦点环按机器为窗口、控件面、边框和高亮保留的那几个颜色
  画，而不是页面自己声明的那几个；复选框那张图仍旧是工程画出来的像素。这个开关属于坐在机器前的人，
  所以用例把答案写进这次运行的进程，而不是替用户打开它，两种答案各跑一遍。此外只验到消息这一层：
  用户开关开关或换一套配色时 Windows 会给窗口发消息，用例只确认窗口收得下这条消息、并继续画它本来
  那一帧——在它手上这个答案变不了。
...
把跳过变成失败。要驱动窗口的那些用例还要求可交互的桌面会话：以服务方式启动的进程没有窗口站，
开不出窗口，那里同样跳过，`NANO_INSTALLER_E2E_REQUIRE_DESKTOP=1` 能把这次跳过也变成失败。
其中光标用例要的比一个窗口更多：只有真的在显示指针的会话才回答得了它。托管 runner 有窗口站、
画得出窗口，却没有鼠标，读回来的是空光标，所以那条用例在那里打印自己的跳过理由。桌面被锁屏时
（会话里跑着 `LogonUI`，前台是锁屏界面），指针读回来是箭头、指针移进移出也不再影响任何窗口，
这两条会如实失败而不是跳过——它们问的就是真的动过指针之后窗口的样子，跳过等于没说。

给构建好的安装包签名属于发布流水线，不属于这套测试：`scripts/sign.ps1` 用
`NANO_INSTALLER_CERT_THUMBPRINT` 指定的证书签名并校验。构建器只提供两个钩子，把成品按顺序交给工程
自己写下的命令（卸载程序先、安装包后），命令失败就停下；套件在两侧各钉一条：一条证明命令真的在成品
上跑过、被处理过的安装包照常安装，一条证明命令失败时构建中止、不留安装包。测试盯住的是证书改变不了
的那部分——签名追加进去之后，文件仍读得到自己的资源。

## 截图快照

快照要桌面会话的理由和窗口用例一样，所以它在有桌面的机器上跑，不放在构建代理上。

`scripts/capture_setup_snapshots.ps1` 会构建示例安装包，把工程声明的每一页都拍下来。向导的第一页是读者
真正会看到的那一页，所以它每个支持的语言各一张，另加每档更高缩放各一张（默认 150% 与 200%）；其余页面各拍一张。
一个安装包只会打开 `wizard.pages[0]` 指的那一页，所以每拍一页之前，脚本先把这一页临时排到第一位再构建，
拍完整份工程配置按原字节写回：
...
- 客户区与页面声明一致，缩放时按自己的倍数放大；每一页按自己的版面，卸载页是 574x358。
- 版面声明了圆角的页面，四个角都在窗口区域之外，也就是角是被切掉的。
- 布局里用绝对坐标摆下的每个图片和标签都画出了东西：图片不是一块纯色，标签在自己声明的颜色下有文字像素。
- 整张图的颜色数远多于一张空白页。
- 各页的画面互不相同，说明拍的确实是那一页，而不是每张都拍成了第一页。
- 第一页的按钮和版本行在不同语言下像素不同，说明语言确实传到了页面上。

拍的是版面画出来的样子，不是走完流程走到那一步：卸载页并没有真的卸载过，流程走到那一步会发生什么，
由「页面与控件」那批用例守着。payload 里有没有东西并不重要——页面在解包之前就画出来了。
示例的 payload 是 150 MB 的归档，仓库不跟踪它，没解包过示例的机器上没有这个文件，
脚本会用一个同格式的空归档来构建，跑完再删掉。
...
上面那些快照已经核对过客户区、圆角、布局摆下的图片和文字，以及第一页的按钮和版本行是否跟着语言变。剩下的得有人在机器前看：
