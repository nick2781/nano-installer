...
`cargo test --locked --workspace` runs 228 cases: 169 in the core library, 23 that build a real
...
| --- | --- | --- | --- |
| Core library | 169 | what a page becomes — layers, coordinates, hit regions, text — what the bundle carries, what an install writes to disk and the registry, and what each script primitive does | that a packaged setup reaches any of it |
