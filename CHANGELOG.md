...
- **有客户程序连着时，改过输入框的那一页会把向导弄崩**（`0xc000041d`，Windows 说这是「回调里逃出来的
  异常」）。复现与已经探明的边界都写在那条被忽略的用例
  `a_client_reading_the_page_does_not_take_the_wizard_down` 上：不连客户程序，或者让客户程序拿到窗口
  自己那份回答，同样的按键与页面都活着；连上之后，光连接与提问也活着，把输入框填好再清空、然后再问
  一次就死。窗口收到的消息停在 `WM_PAINT`，画的就是回来的那一页；装一个 panic 钩子什么也没写，所以
  异常出在原生代码里，不是 Rust 的 panic。手上没有调试器，因此还没有调用栈；下一步是拿一份崩溃转储
  （开 WER LocalDumps，或者装 WinDbg/cdb）看是谁在回调里出错了。
