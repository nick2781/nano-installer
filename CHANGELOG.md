...
- 屏幕阅读器眼下读到的是页面本身，还没有进度播报：正在安装时那行状态文字与百分比不主动念出来，
  用户要自己去问；输入框里每敲一个字符也不逐字播报（进到字段时念出字段名，读一次当前的值）；
  通过 `IDispatch` 晚绑定的客户程序还没被服务，走的是 `IAccessible` 接口本身——Windows 自带的
  阅读器、以及把 MSAA 接到 UI Automation 下面的那座桥，走的都是这个接口。
