...
- 卡住的根因找到了：套件自己发的「阶段」状态就是那个没有期限的调用。状态由 `Invoke-RestMethod` 发出，
  ...
  看门狗自己发状态用 `HttpClient`（它的超时是覆盖响应体的），因为它只有这一件事，栽在同一个缺陷上
  就等于把记录丢两次。
...
- `a_package_installs_the_product_the_setup_carries_and_removes_it_again` 在整套用例一起跑时失败过一次
  （失败信息是「the product is still on disk after the package was removed」），单独跑 13.6 秒通过。
  卸载收尾本来就是异步的：卸载程序把自己复制到临时目录，等自己退出之后才删安装目录，最后让一个
  `cmd` 脚本删掉自己，`msiexec /x` 返回时这串动作可能还没走完。等有空时让这条用例在断言前等目录
  消失（带期限），而不是假定它立刻消失。
- 安装包尚未签名，Windows SmartScreen 仍会提示「未知发布者」。
- 卡死还没有被证明已经消失：本机跑完整套套件是绿的（366 条、约七分钟），而 CI 上卡住的那一轮停在
  第一条命令上，符合「冷缓存 + 单条期限」这条因果链，处理也已经按它做完了（先编译、后跑测试，
  缓存键升到 `ci-v3` 让下一轮就是冷启动）。若冷启动那一轮仍然卡住，`suite phase` 会说出它停在哪个
  target、那条命令最后一行是什么，那时候再按那行查。眼下的自动化只做到「恢复」：下一轮运行的
  supersede 会强制取消卡住的那一轮，ci-janitor 也会为超过 30 分钟的运行另起一轮（实际触发频率远低于
  每 15 分钟一次，近期几小时只跑到三次）。
