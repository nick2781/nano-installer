...
- 输入框里敲进去的每一个字符都会被念出来。以前只有「别的东西改了字段」（选目录对话框、脚本写入）才发一次
  取值变化通知，用户自己打字这条路上什么都没发——读屏于是在打字时是哑的，只在进到字段时读一次当时的值。
  现在每一次字符输入、删除、粘贴、剪切、撤销、重做都会发一次取值变化通知（客户程序据此重读字段），而
  只在文字里移动光标的按键一次都不发：取值的比较就写在发通知之前，因为方向键是打字时最常按的键，每按
  一次就重念一遍整个字段，比不念更难用。
- 走 `IDispatch` 晚绑定的客户程序也被服务了。以前它们只拿到 `E_NOTIMPL`：名字换不成编号、成员也调不动。
  现在 `GetIDsOfNames` 认得 MSAA 的那 18 个成员名（`accName`、`accRole`、`accState`、`accLocation` 这些），
  `Invoke` 把它们逐个接到 `IAccessible` 同一个实现上——晚绑定是「怎么调这个接口」，不是第二个接口，
  所以两条路给出的答案必须一致（用例就是这么比的）。老实的部分也照旧老实：MSAA 没有的名字返回
  `DISP_E_UNKNOWNNAME`，这个对象本身不服务的成员（`accSelection`、`accHelpTopic`）返回它们各自的
  拒绝，不编一个空答案糊弄过去；`accParent`/`accChild` 交回的对象在 VARIANT 里是 unknown 而不是
  dispatch 指针——这一处与接口签名不同是有意的，从这一侧没有安全办法造出 dispatch 类型的 VARIANT，
  而客户程序要调用它本来就要 QI 一次 `IDispatch`。
...
- 听窗口事件的那几条用例现在一次只跑一条。钩子和它填的那张表是进程级的，被监听的那个窗口也是——
  两条用例同时听，就会各自收到对方页面的事件：键盘那条用例因此在旁边多了一条监听用例之后，读到了
  另一个窗口的焦点事件（它期望 `[1]`，读到的却是 `[0]`）。这四条用例因此和指针那两条一样轮流拿一个
  锁（`the_listener`），谁先跑到谁先听。轮流之后它仍然失败过一次，而且这一次是单独跑的：`[0]` 不是
  别人的窗口，是 Windows 把「窗口自己拿到键盘」也报成一次焦点事件——子项 0 说的是窗口本身，不是一个
  控件。所以这条用例现在只看真正移动到的控件：子项 0 滤掉之后，`[1]` 才读作「键盘走到了第一个控件」，
  两个 Tab 读作 `[1, 2]`。
- 生产状态文档里那组用例数是旧的：它写着「58 个用例里有 17 个会开出向导窗口并驱动它」，而这一版是
  69 条、其中 25 条会开出向导窗口并驱动它。数字偏小等于把后来加进来的用例说成不存在，读这份文档的人
  会据此以为覆盖面比实际窄。两份语言现在都按实测写（69 条里 25 条开窗、44 条不开），并把此前没有被
  列进那段话的八条读屏用例补进去——它们本来就是那 25 条的一部分。
...
- 卡死还没有被证明已经消失，但已经有实测的处置：近期六次运行里五次停在核心库那条单元命令上，且都是
  ...
  加 `-- --test-threads=1` 之后它跑完，这一次运行两个 job 全绿；本机整套套件也是绿的（373 条、0 失败、
...
- 这一版收尾时的验证：本机整套套件 373 条全跑到（370 通过、2 失败、1 忽略；会话在锁屏状态时，按
  `docs/*/TEST_PLAN.md` 的约定，需要桌面指针的两条会如实失败——这一次就是如此，安装包级的 67 条其余
  全部通过，包括这一版新加的两条可访问性用例；上一次会话未锁屏的整轮是 0 失败。核心库那条改成一条
  用例一跑之后从 3 秒变成 17 秒），`cargo fmt --all -- --check`、
  `cargo clippy --locked --workspace --all-targets -- -D warnings` 与三个审计脚本（用例描述 373/373、
  测试目标、脚本编码）全部退出码 0。
