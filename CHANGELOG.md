...
- 步骤启动命令时不再把自己的输出交给子进程。构建机给每一步的是一根输出管道，并在这根管道关闭时才
  ...
  启动时这一步的输出在它自己退出后 7.5 秒才关，按新办法（`CreateProcess` 且
  `bInheritHandles = false`，自带控制台）立刻关。`step_job.ps1` 因此多了一个 `NanoStepCommand`，
  `run_tests.ps1`、`run_e2e_setup.ps1` 与 CI 的构建步骤都用它启动命令；类型加载之后还会收掉工具链
  留下的两个常驻助手（`vctip`、`mspdbsrv`），因为它们是同一类东西：活得比这一步久，就把这一步按住。
