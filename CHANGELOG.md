...
- 卡死的根因找到了：套件自己发的「阶段」状态就是那个没有期限的调用。状态由 `Invoke-RestMethod` 发出，
  ...
  证据也对得上：卡住的那几轮都停在阶段里的**第一条「命令还在跑」的状态**上（此前那些状态都是空闲时发的，
  整条路径从没在 CI 上成功执行过），而 CI 里另一个跑很久、但从不发状态的步骤（`Setup End to End`）
  一次都没卡过。现在发状态的是 `scripts/post_status.ps1`，一个独立进程，调用方给它 15 秒，不答就
  整个结束掉——记录套件停在哪里这件事，不能自己成为套件停下的原因；被结束掉几次会写进报告和最后
  一条状态，一次运行少了哪几条状态因此不会没人知道。
- 看门狗现在结束的是整棵进程树，不只是步骤自己那个进程。步骤是否结束由构建机判定，而构建机等的还有
  ...
  仍然活着（输出照样开着），换成 `taskkill /T` 它跟着没了。看门狗（25 分钟）因此从「结束一个进程」
  变成「结束一棵树」：卡死的那一步现在是花掉自己的期限、以失败结束并留下日志，而不是一直挂到有人
  强制取消。
