...
- 输入框破坏规矩时那句提示也会被念出来。页面上每一个 alue-source="field-error:<字段 id>" 的标签都是「哪条规矩没过」的说明，它只在破坏规矩时画出来；现在它同时被记成字段自己的描述（客户程序问这个字段「还有什么要说的」，拿到的就是这句），并在出现或换成另一条规矩时按活动区域播报——用户看不到按钮为什么是灰的，这一句是唯一的解释。提示消失不播报：值合格了，没有话要念。
