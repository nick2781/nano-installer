...
- 卡住的测试步骤现在会留下它停在哪一条命令。工作流历史里这类运行并不少见：最近 15 次 main 上的 CI
  ...
  阶段因此另外发一条提交状态（上下文 `suite phase`，内容带上时刻与阶段名，每跑一个 target 就更新
  一次）：提交状态不随运行结束而消失，被取消的运行因此仍然说明它停在了哪一个 target 上。这条状态
  故意写成 success，它是一块告示牌而不是结论——结论仍由作业自己的检查给出，而留一条 pending 会被
  读成一个从没通过的检查。
...
- 测试步骤仍会卡死，而且卡住的作业连日志都没有。本机把整套套件跑完是绿的（366 条、约七分钟），
  所以还不知道是哪一个 target、停在哪一步；`suite phase` 那条提交状态就是为下一次卡住准备的——它会
  点出停在哪个 target，然后才能对症处理。眼下的自动化只做到「恢复」：下一轮运行的 supersede 会
  强制取消卡住的那一轮，ci-janitor 也会为超过 30 分钟的运行另起一轮（实际触发频率远低于每 15 分钟
  一次，近期几小时只跑到三次）。
