...
- 换过启动方式之后，两份报告在本地重新跑通：`run_tests.ps1` 退出码 0，报告里核心库 210 通过、
  1 忽略，安装包级 40 通过、2 过滤，工程检查 5，构建器 29，两个运行时各 1；
  `run_e2e_setup.ps1 -RequireDesktop` 40 通过、2 失败，失败的仍是那两条需要真实指针的用例
  （工作站此时锁屏）。两份报告都照旧写出，`提权运行` 一栏如实写着「否」。
