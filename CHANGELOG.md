...
- 企业分发的 MSI 包也能在构建里签名了：配置里多一个 `finalize.package`，构建写出 `.msi` 之后把那个文件
  交给工程自己的命令（和 `finalize.installer` 同一个写法，`%1` 是包的完整路径）。签名会改写文件，所以
  构建日志与结果里的包大小是命令跑完之后从磁盘上量的；命令退出码非零就中止构建，那份包也不会留在磁盘上。
...
- 用屏幕阅读器的用户能听见向导里的这一页。运行时把控件都画成像素，客户程序顺着子窗口找下去什么也
  找不到，所以窗口自己回答 Windows 唯一会问的那句「你在显示什么」：页面上的控件按版面顺序报出去，
  每个都带上它是什么（按钮、链接、复选框、单选按钮、下拉框、输入框）、叫什么（版面写下的文字；
  输入框取它上方的标签，下拉框取它收起时显示的那一行）、此刻是什么状态（有没有被选中、有没有拿到
  键盘、下拉框能展开）、在屏幕上的哪里（客户程序据此画出高亮框），以及按下它会做什么——客户程序
  请求执行默认动作时走的就是用户点一下的那条路，请求把键盘交给某个控件就是 Tab 走到它。清单变动、
  取值变动和翻页时窗口主动发出通知，用户每按一次 Tab 都会被念出来，不必等阅读器再问一遍。对话框
  打开时整棵树换成卡片上的两个答案，窗口的名字就是那句提问。

- 输入框破坏规矩时那句提示也会被念出来。页面上每一个 `value-source="field-error:<字段 id>" 的标签都是「哪条规矩没过」的说明，它只在破坏规矩时画出来；现在它同时被记成字段自己的描述（客户程序问这个字段「还有什么要说的」，拿到的就是这句），并在出现或换成另一条规矩时按活动区域播报——用户看不到按钮为什么是灰的，这一句是唯一的解释。提示消失不播报：值合格了，没有话要念。
- 正在装的那一步也会被念出来。安装跑起来之后页面上已经没有可以点的东西了，剩下的就是任务自己报的
  那句话和一根进度条，而这两样原先根本不在读屏能看到的清单里：现在它们作为「一行状态文字」和
  「一根进度条」被报出去（进度条的值是它画着的百分比），并且每变一次都由窗口主动播报——状态文字
  按活动区域播报（客户程序听见就念，不必先来问），进度条报成取值变化（每百分之一都念会吵得没法用）。
  播报按「变了才报」发：页面重画的次数远多于任务报进度的次数。工程脚本的 `set_status`、
  `set_status_key`、`set_progress` 与运行时内置的每一步走的是同一条路。

- 向导可以用键盘走完一整页。`Tab` 与 `Shift+Tab` 按版面声明控件的顺序走遍带动作的控件与能输入的
  输入框，走到两头就绕回来；键盘落在哪个控件上，就在那个控件自己的矩形上画一圈一像素的点状环，颜色
  取这个控件的 `focus-color`、再取页面的、最后是默认强调色。`Enter` 与空格做环底下那个控件会做的事，
  环圈住输入框时同时把光标交给它，可以直接打字；鼠标点中的控件也拿到环，键盘接着从刚点过的地方走。
  被禁用的按钮、只读字段、隐藏的元素、没有动作的标签和没有 `id` 的输入框都进不去这段路，菜单或
  对话框打开期间键盘归它们。

- 用户打开高对比时，版面声明的颜色让位给系统配色。运行时画出来的页面是一张图，Windows 不会往里面
  画自己的控件，所以颜色按它画的是什么交给系统：页面与弹层用 `COLOR_WINDOW`，卡片、控件自己的面、
  进度条轨道与滚动条滑块用 `COLOR_BTNFACE`，描边用 `COLOR_WINDOWFRAME`，页面上的字用
  `COLOR_WINDOWTEXT`、按钮上的字用 `COLOR_BTNTEXT`，焦点环与键盘所在的那一行菜单用
  `COLOR_HIGHLIGHT`、其上的字用 `COLOR_HIGHLIGHTTEXT`，滚动条轨道用 `COLOR_SCROLLBAR`。角色分开
  是必要的：Windows 自带的高对比主题里页面颜色与控件面色常常相同，而按钮上的字与页面上的字往往不同。
...
- 卡住的测试步骤现在会留下它停在哪一条命令。工作流历史里这类运行并不少见：最近 15 次 main 上的 CI
  里，「Native Win7+ Build」有 8 次被下一轮取消、2 次失败，只通过 1 次（同一时间「Setup End to End」
  13 次通过），而卡住的作业什么也没留下——作业被强制取消时，它那一份日志根本不会上传。套件的每个
  阶段因此另外发一条提交状态（上下文 `suite phase`，内容带上时刻、阶段名，以及运行中那条命令最后
  写下的一行）：提交状态不随运行结束而消失，被取消的运行因此仍然说明它停在了哪一个 target、那一条
  命令最后在等什么。这条状态故意写成 success，它是一块告示牌而不是结论——结论仍由作业自己的检查
  给出，而留一条 pending 会被读成一个从没通过的检查。它也换来了根因：卡住的那一轮停在**第一条**命令
  （`cargo test -p nano-installer-core --lib`）上。
- 套件把「编译」和「跑测试」分开了，卡死因此有了对症的处理。冷缓存下第一条命令要先把整棵依赖树
  编出来，这比「某条命令不再答话」的期限长得多——10 分钟的单条期限于是正好落在编译上，跑进那个
  本该结束卡死步骤的分支，而这个分支自己又卡在往里打印上。现在套件先跑一遍
  `cargo build --locked --workspace --tests`（自己的 40 分钟期限，属于构建），下面每个 target 的
  10 分钟期限才真正是在管跑测试。缓存键随之升到 `ci-v3`：换键后的那一轮就是冷启动，而冷启动正是
  套件必须活下来的那种情况。
- 向导的指针与键盘行为第一次有自动化用例看守：悬停与按下换上的状态位图、按钮上的手型与输入框上的
  ...
  没有鼠标，它会在那里打印跳过理由。
- 可视化构建器的参数页也能要 MSI 包了：勾上之后这次构建同时写出企业分发的 `.msi`，路径默认跟着
  输出 exe 走（同名 `.msi`），自己改过就不再跟着变；下面那行等价命令行会带上 `--msi`，构建结果里
  也报出包的大小。没勾的构建和以前一样，请求里既没有包、预览里也没有这个开关。
- 流水线自己结束被替代的运行。`Test workspace` 这一步偶尔永远不结束，原因在构建机上：步骤要等自己
  的输出关闭才算结束，而这一步启动的某个进程有时一直握着它，构建机上任何超时都够不着这样一步；它
  占着并发组不放，于是下一次推送只在它后面排队，而不是开始构建。新的 `supersede` job 在两个构建
  job 之前把这些还在进行、还在排队的同分支运行强制取消掉，并且允许失败——取消被拒不能让一次本来
  全绿的构建变成红的。
- 期限分支不再问 WMI。它要写下的那份「当时还活着的进程」原本来自 `Get-CimInstance Win32_Process`，
  而这个调用本身可能在机器不顺时无限期卡住：本该结束一次卡死步骤的分支，于是把那个步骤一起按在原
  地，连日志都留不下。现在它读进程表（pid、进程名、启动时间、CPU、窗口标题），一定有界；命令行只有
  WMI 拿得到，这里有意舍弃——它买不来一个不再卡住的步骤。
- 生产状态那份文档（`docs/{en,zh-CN}/PRODUCTION_STATUS.md`）的头一句不再是笼统的「当前实现尚不能用于
  生产发布」，而是写清真正缺的是什么：Windows 7 SP1 一次都没有被观察过，其余每一条都在 Windows 11 与
  CI 上验证过，并指向「阻塞发布的问题」那一节。另加一节「发布」，把上面两道查出来的关口写在这里——
  读这份文档的人要能一眼看出「可以依赖什么、还差什么」。
- 期限分支收进程树也带上期限。`taskkill /T` 要等的正是可能把一切按住的那棵树，所以它和这条分支里
  别的调用一样有界：两分钟收不掉就交给这一步加入的 job，脚本进程一退，它就跟着结束。`step_job.ps1`
  因此多了一个 `Stop-ProcessTree`，两个脚本共用。
- 发布只从套件绿过的提交发出。tag 是手推的，发布 job 自己不跑套件，「这个提交测过」此前只是一个假设——
  而本仓库的运行经常因为下一次推送被取代（结论是 cancelled，不是结论），所以这个假设经常站不住。
  现在发布 job 在动手构建之前先问一遍：这个提交上最新的一次 **push** 运行是不是成功
  （`scripts/audit_release_evidence.ps1`）；不是就当场拒绝，几秒钟结束，而不是先构建二十分钟再发出去。
  派发的运行不算数，因为派发的运行可以只跑指定的 target。
- 发布带上摘要清单。`scripts/release_manifest.ps1` 写出 `SHA256SUMS.txt`（5 个可执行文件各一行，
  格式是 `sha256sum -c` 认的那种），随 release asset 一起上传。下载是安装里唯一事后没人能检查的一步，
  而安装包自己拉依赖时本来就要求摘要对得上——它自己发布的文件现在也给了同一条规矩。

### 修复

- 空输入框不再把向导画崩。有读屏客户程序连着时，把输入框填好再清空、然后再问一次页面，向导进程会以
  `0xc000041d`（回调里逃出来的异常）结束。Windows 的事件日志把故障点写在 `user32.dll` 里 `DrawTextW`
  的范围中（偏移 `0x24a82`，异常 `0xc0000005`）；按「每次画一段文字就记一行」追下去，最后一行正是
  **一个字符都没有** 的那一段——空输入框画出来的那段。空 `Vec<u16>` 拿到的指针是悬垂指针，
  `DrawTextW` 在排版路上会去读它。现在没有字符的片段直接跳过。这条路径本来就在每个输入框一开始、
  以及用户清空之后的每一帧上跑，只是要有客户程序连着才稳定复现。
- 测试套件按 target 拆开跑。`run_tests.ps1` 现在一条命令一个 target（核心库单元、e2e、工程检查、GUI、
  CLI、两个 stub、卸载程序、doc-tests），每条命令各有自己的期限：哪个 target 停住，日志里就是哪条命令
  没结束——之前整个套件是一条命令，卡住时只能猜。一条 target 失败也不再挡住排在它后面的 target。另外，
  命令的输出文件改成按共享方式读：命令自己的子进程可能还握着它，之前这会直接让整次运行报错退出。
- 缓存可能是卡死的原因，换一次 key 就能丢掉旧的那份。被取消的运行永远走不到「保存缓存」那一步，下一次
  运行却把上一次保存的 `target/` 解包回来接着用；如果那次保存是被打断的，恢复出来的就是一棵 cargo 会在
  上面停住的树。`ci.yml` 的两个缓存步骤因此加了 `key: ci-v2`（把旧缓存整批丢掉一次）：换 key 之后那次运行
  的 `Test workspace` 103 秒通过，而在此之前用旧缓存的连续六次都卡 19 到 28 分钟不结束。这是一次观察而不是
  证明，所以它写在文档里：再遇到「套件起来之后不再应答」，先换一次缓存 key 看看。
- 不再应答的步骤由外面的看门狗结束。一个步骤是否结束取决于它的输出有没有关闭，而不再应答的步骤永远不会
  关闭它，构建机上任何超时都够不着——步骤自己内部的那条期限分支也在步骤里。`step_job.ps1` 因此多了一个
  `Start-StepWatchdog`：它在步骤之外起一个进程（走同一套启动器，所以它自己不持有这一步的输出），到点还没
  结束就把这一步的进程结束掉。卡住于是只花掉它的期限，而不是构建机愿意等的一小时，而且这次失败会带着日志
  留下来。
- 卡住的流水线会自己重新开一次。`Native Win7+ Build` 的 `Test workspace` 偶尔永远不结束——构建机上
  任何超时都够不着它。新的 `CI janitor` 工作流每十五分钟检查一次：凡是在跑的 CI 运行超过 30 分钟，就在同一
  分支上派发一次新的运行；新运行自己的 `supersede` job 会把卡住那次收走，所以定时任务本身不需要取消任何
  东西，也不需要有人推一次提交。
- 期限分支读命令的输出时只读「现在已经写下的那些」。原来它读到文件末尾，而永不结束的命令不会停止写：
  一个跑不完的测试会让套件每分钟写一行，于是这个读取永远不结束，本该结束卡死步骤的分支就和它一起卡在
  那里。现在读的是打开文件那一刻的长度。
- 步骤启动命令时不再把自己的输出交给子进程。构建机给每一步的是一根输出管道，并在这根管道关闭时才
  认为这一步结束；`Start-Process` 与 `Process.Start` 无论走哪条路都会把当前进程所有可继承的句柄
  交给子进程，于是一个活得比命令久的进程就把这一步永远按住。实测：让一个孙进程活 8 秒，按老办法
  启动时这一步的输出在它自己退出后 7.5 秒才关，按新办法（`CreateProcess`、`bInheritHandles = false`
  且不给控制台）立刻关。`step_job.ps1` 因此多了一个 `NanoStepCommand`，`run_tests.ps1`、
  `run_e2e_setup.ps1` 与 CI 的构建步骤都用它启动命令。命令不再有自己的控制台：控制台要由系统先建
  好进程才能跑，那一步握手本身就是步骤可能停住的地方，而命令的输出本来就由外壳重定向到文件。
- 同一天再发一版的 MSI 装得上了。以前同一天的第二版与第一版共用产品码，Windows Installer 会以 1638
  「已安装这个产品的另一个版本」直接拒绝它，`/i` 连装都装不进去；现在产品码按 `project.version`
  **写下的样子**推导（`2026.9.17` 与 `2026.9.17-r2` 是两个），升级表的上限改成含等号（`513`：含等号
  是 512，而 2 是只检测——找到旧产品却什么都不收），于是第二版先把第一版收掉再装上，机器上始终只有
  一份产品，卸载也还是普通的一次卸载。
...
- 卡住的根因找到了：套件自己发的「阶段」状态就是那个没有期限的调用。状态由 `Invoke-RestMethod` 发出，
  而 Windows PowerShell 把它实现在 `HttpWebRequest` 上，`Timeout` 管的是拿到响应，不管**读完响应体**——
  本机实测：让一个服务端把响应头写好、正文一个字节都不发，`-TimeoutSec 10` 的调用 45 秒后仍在等。
  证据也对得上：卡住的那几轮都停在阶段里的**第一条「命令还在跑」的状态**上，连「这条命令超了期限、
  停在某一行」那条都没能发出来（它走的是同一个调用），而 CI 里另一个跑很久、但从不发状态的步骤
  （`Setup End to End`）一次都没卡过。记录因此搬出了套件：套件只把「我在哪」追加进一个 crumbs 文件
  （本地写文件，写在每一件可能阻塞的事**之前**），由步骤之外的看门狗 `scripts/step_observer.ps1` 读它、
  读正在跑的那条命令的输出文件，再把两样发成提交状态。`suite phase`（这一步说了什么）、`suite output`
  （命令最后写下的那一行，也就是一直没回答的那条用例名）、`suite step`（步骤进程还在不在、用了多少
  处理器时间、安静了多久）三个上下文都留着；机器本身没了就一条也发不出来，那份沉默本身就是结论。
  看门狗自己发状态用 `HttpClient`，但真正的那道界线在它自己手里：每次最多等 10 秒（`Wait`，不是客户端
  的超时——连接被黑洞掉的请求可以让任何客户端的超时都不生效），等不到就丢掉这一次、继续下一轮，
  并把丢掉的次数写进下一份健康报告。网络没了这件事于是说得出来（`N post(s) unanswered`），而不是
  只剩沉默。本机用「收下请求、永不回答」的服务端验过：观察者在那种情况下仍按自己的期限跑完
  （216 秒），把每一步都记下来，并在最后一轮把丢掉的次数报出来。
- 套件里第一条命令（核心库单元用例）现在一次只跑一条用例。构建机上卡住的运行有六次里的五次停在
  这条命令上，且都是开始后几秒；同一提交、同一个 runner 镜像在几小时前是绿的，说明机制在构建机上
  而不是在这份提交里。这条命令里有几条用例真的动机器而不是只动临时目录——装一个服务、写一个整机
  共享的键、问「这次运行到底有没有权限这么做」——而构建 job 的这一步是**提权**跑的，开发机上普通
  的运行到不了那些分支，所以本机复现不出来。实测：加 `-- --test-threads=1` 之后同一条命令跑完，
  这一次运行两个 job 全绿。这是把「别同时做那几件事」写进套件，而不是证明机制已经消失——真正的
  机制还在构建机那边，`suite_command` 那个派发入口就是留给下一次查它用的。
- 看门狗现在结束的是整棵进程树，不只是步骤自己那个进程。步骤是否结束由构建机判定，而构建机等的还有
  这一步的输出：一个继承了输出的子孙进程会让输出在步骤进程退出之后依然开着。本机实测：步骤进程立刻
  退出、一个孙进程活 25 秒，这步的输出在 25.2 秒后才关；只对步骤自己用 `Stop-Process` 时那个孙进程
  仍然活着（输出照样开着），换成 `taskkill /T /F` 它跟着没了（收树这件事本身也有期限，两分钟收不掉
  就交给这一步加入的 job）。卡死的那一步现在是花掉自己的期限、以失败结束并留下日志，而不是一直挂到
  有人强制取消。
- 每分钟那条进度只说「还没有输出」。它读的是文件最后一行，而最后一行永远是空的——文件最后那次写入
  以换行结尾。于是这条本该说出「卡在哪条用例」的记录，一路以来只会说「no output yet」。现在读的是
  最后一条**有内容**的行（与期限分支同样的读法），所以一条命令跑过一分钟之后，每一分钟都会报出它
  最后写下的那一行。
- 测试步骤的日志瘦下来了。`run_tests.ps1` 多了 `-Quiet`：CI 只打印每个 target 的结果行（失败时再补上
  它最后 40 行），整份输出仍旧写进 `target/test-report.txt` 与 `.html`，也就是作业上传的那份产物——
  报告是文件，控制台是管道，管道可能没人读。命令输出被读回时也只读末尾 256 KB：卡住的命令会一直写，
  一个必须读到结尾的读取就是又一个把步骤按住的理由。
- MSI 卸载后不再抢在卸载程序前面断言。`a_package_installs_the_product_the_setup_carries_and_removes_it_again`
  在整套用例一起跑时失败过一次（「the product is still on disk after the package was removed」），单独跑
  13.6 秒通过——因为收尾本来就是异步的：卸载程序把自己复制到临时目录，等自己退出之后才删安装目录，
  最后让一个 `cmd` 脚本删掉自己，`msiexec /x` 返回时这串动作可能还没走完。三条走 `msiexec /x` 的用例
  现在都先等目录消失（带期限，30 秒），再断言它不在，和其余几条卸载用例用的是同一个等待。
- 报告里「跳过的用例」那一节此前永远空着。用例自己打印的跳过理由落在测试框架的输出里，而框架默认把
  通过用例的输出收起来，脚本于是根本听不到。`run_tests.ps1` 与 `run_e2e_setup.ps1` 跑 e2e 那条命令时
  因此带上 `-- --nocapture`：一条用例为什么离开，是当着读报告的人说的，而不只是写在用例的代码里。

### 已知限制

- 两条由页面控件决定安装目录的端到端用例在 CI 上偶尔失败（`a_field_the_user_fills_in_is_what_lets_the_install_start`、
  `a_click_on_a_radio_is_the_value_the_install_waits_for`，失败信息是「安装没有写进配置的目录」）。本机反复跑整个
  套件没有复现过，它们对时序敏感——窗口里输进去的值要赶上安装开始的那一刻。这两条现在把等待从 20 秒放到
  30 秒，并且在失败信息里带上当时的证据：输入框那条说出**字段实际持有的值**，两条都打印页面此刻的描述
  （每个控件的角色、名字、取值、状态、位置）。下一次在 CI 上失败时，那条信息会直接说明是敲进去的字符
  没到、还是那一次点击没被记下、还是安装本身停在了别处，而不用再猜。
- 安装包尚未签名，Windows SmartScreen 仍会提示「未知发布者」。
- 卡死还没有被证明已经消失，但已经有实测的处置：近期六次运行里五次停在核心库那条单元命令上，且都是
  它开始后几秒（同一提交、同一个 runner 镜像在几小时前还是绿的，所以机制在构建机上）。给那条命令
  加 `-- --test-threads=1` 之后它跑完，这一次运行两个 job 全绿；本机整套套件也是绿的（371 条、0 失败、
  1 忽略，约九分钟，核心库那条从 3 秒变成 17 秒）。构建机上到底发生了什么仍未确定，判据留在
  `suite phase` / `suite output` / `suite step` 三个提交状态里：再停住时，最后一条状态会说它停在等命令、
  读输出、还是发状态上，「命令最后写下的那一行」就是那条没回答的用例名，而「N post(s) unanswered」
  说明网络本身没了。恢复的自动化照旧：下一轮运行的 supersede 会强制取消卡住的那一轮，ci-janitor 也会
  为超过 30 分钟的运行另起一轮（实际触发频率远低于每 15 分钟一次）。
- 屏幕阅读器眼下读到的是页面本身加上任务报的进度：输入框里每敲一个字符还不逐字播报（进到字段时念出
  字段名，读一次当前的值）；通过 `IDispatch` 晚绑定的客户程序还没被服务，走的是 `IAccessible` 接口
  本身——Windows 自带的阅读器、以及把 MSAA 接到 UI Automation 下面的那座桥，走的都是这个接口。
- MSI 的版本号只有 `主.次.构建` 三段（Windows Installer 自己只比较这三段），`project.version` 里的
  修饰后缀与第四段在这里被丢掉，三段都必须能读成数字。同一天再发一版（`-r2`）因此与上一版的 MSI
  版本号相同：这两版是两个产品（产品码按写下的版本号推导），升级表把「等于当前版本」也算作要收走的
  旧产品，所以第二版会把第一版替换掉——但要让机器认得出版本号变了，仍然得让三段数字真的变。
...
- MSI 不手写复合文档，而是让 Windows Installer 自己的数据库引擎来写：`MsiOpenDatabase` 建库，包自己
  ...
  表：`Remove` 列必须留空（写成空字符串会什么都不删），上限取当前版本且含等号（512 是含等号，2 是只
  检测：找到旧产品却什么都不收），下限留空以覆盖所有更早的版本；同一天再发一版时三段数字没变，那一版
  靠产品码与这条含等号的边界把上一版收走；`RemoveExistingProducts`
...
- 键盘走的那串控件就是排版自己记下来的：每个带 `WindowAction` 的控件与每个可编辑输入框，在排版走过
  文档树时按顺序连同矩形记成一个 `FocusRegion`，所以用户走的顺序就是页面显示的顺序，没有第二张要与
  版面保持同步的表。环是画在控件自己矩形上的 1 像素点状层，排在页面内容之后，颜色取控件的
  `focus-color`、再取页面的、最后是向导的强调色。往回走在运行时是向 Windows 问 Shift，窗口用例投递
  的按键消息带不上修饰键，所以 Shift+Tab 只有单元用例（`docs/{en,zh-CN}/TEST_PLAN.md` 写着这一条）。

- 正在跑的任务报出来的话和进度条也是「被描述的东西」，但它们不是控件：排版遇到 `ProgressBar` 和
  绑定 `value-source="status"` 的文字时，除了画出来再记一条 `LiveRegion`（种类、文字或百分比、
  矩形），它们排在可键盘到达的控件之后——进度页通常一个控件都没有，客户程序要念的就是这两样。
  广播走 `update_runtime` 之后的 `WM_APP_REFRESH`，也就是窗口线程重画的那一处：`RuntimeState` 里
  留着上一帧报过的值，只有变了才发事件，因为页面重画的次数远多于任务报进度的次数。状态文字发
  `EVENT_OBJECT_NAMECHANGE`（它说的就是它的名字）加 `EVENT_OBJECT_LIVEREGIONCHANGED`（让客户程序
  不必先问就念出来），进度条只发 `EVENT_OBJECT_VALUECHANGE`。第一帧只记录不播报：那是阅读器打开
  窗口时自己会读到的内容，否则同一句话会被念两遍。状态文字与进度条在 `accState` 里不带
  `STATE_SYSTEM_FOCUSABLE`，客户程序因此不会以为它们能到达。

- 屏幕阅读器接的是 MSAA，也就是 Windows 从 Windows 95 起就一直在问的那套接口。窗口收到
  `WM_GETOBJECT`（只认 `OBJID_CLIENT`，窗框与标题栏仍交给系统）时，用 `LresultFromObject` 交出一个
  `IAccessible`；跨进程的封送由它和 COM 一起做，客户程序在自己进程里问，回答的是向导进程里的状态。
  新模块 `crates/nano-installer-core/src/accessibility.rs` 用 `#[implement(IAccessible)]` 实现这个接口，
  另加 `implement` 特性与 `windows-core` 依赖（宏写的是 `::windows_core` 路径）。对象每次被问都从
  `RuntimeState` 现算一份清单，不缓存：`FocusRegion` 因此多了 `kind`（版面标签决定角色）与 `name`
  （版面写下的文字，按语言表解好）；输入框的名字取它上方同栏最近的一行文字（版面没有标签属性，
  就近取是唯一的办法，太远的行按字段高度的三倍划断），下拉框的名字与取值取它矩形里画着的那行字。
  子项编号从 1 起、顺序与 Tab 相同——`FocusRegion` 本来就是排版走过文档树的顺序。位置按
  `ClientToScreen` 换成屏幕坐标，因为客户程序拿它画高亮框。事件用 `NotifyWinEvent` 发：`set_focused_control`
  发 `EVENT_OBJECT_FOCUS`，复选框/单选组/下拉框改动发 `EVENT_OBJECT_STATECHANGE`，字段被程序改写发
  `EVENT_OBJECT_VALUECHANGE`，翻页与开关卡片发 `EVENT_OBJECT_REORDER` 再补一次焦点。窗口线程在建立窗口
  之前 `CoInitializeEx(COINIT_APARTMENTTHREADED)`，因为封送要求调用线程进过套间；它活到消息循环结束，
  没有配对的 `CoUninitialize`。对话框打开时清单换成卡片上的答案按钮，窗口自己的名字换成那句提问。

- 高对比就是一个模块加一次参数传递。`crates/nano-installer-core/src/contrast.rs` 把八个角色映射到
  `GetSysColor` 的八个索引；`palette()` 用 `SystemParametersInfoW(SPI_GETHIGHCONTRAST)` 读开关再按
  `HCF_HIGHCONTRASTON` 判断，而不是看颜色像不像系统色——没开高对比的机器照样有窗口色和高亮色，照它
  画就等于把工程自己的美术丢在一边。`LayoutContext` 多一个 `contrast: Option<Palette>` 与
  `colour(role, declared)`：没有方案时借用声明值，所以没开高对比的页面渲染得逐字节与从前一样。窗口收
  `WM_SETTINGCHANGE`、`WM_SYSCOLORCHANGE`、`WM_THEMECHANGED` 时读一次方案，与当前帧用的相同就什么都
  不做，不同才重新排版并重画。用例把答案写进 `NANO_INSTALLER_TEST_HIGH_CONTRAST`（`1` 开、`0` 关、
  未设问机器），因为这个开关属于用户，用例不该替他打开或关掉它；`lib.rs` 的夹具用自己指定的八个颜色，
...
- 这一版收尾时的验证：本机整套套件 371 条全跑到（0 失败、1 忽略，约九分钟；核心库那条改成一条用例
  一跑之后从 3 秒变成 17 秒），`cargo fmt --all -- --check`、
  `cargo clippy --locked --workspace --all-targets -- -D warnings` 与三个审计脚本（用例描述 371/371、
  测试目标、脚本编码）全部退出码 0。两条需要桌面指针的用例这一次也通过（会话当时是解锁的）；按
  `docs/*/TEST_PLAN.md` 的约定，锁屏时它们应当失败而不是跳过。
- 看门狗/观察者（`scripts/step_observer.ps1`）本机验了三件事：对着一个记录请求的替身 API，它把每一步、
  正在跑的那条命令的最后一行、以及步骤进程的健康状况都发成了提交状态；用一个「收下请求、永不回答」
  的服务端时，它按自己的期限跑完 216 秒、一直没被卡住，并把丢掉的次数报在最后一份健康报告里；
  把期限设成 1 分钟时，它把假步骤和它的子孙进程（本来都要活 5 分钟）整棵结束了。
- CI 上的处置有两次实测：同一个 target 用默认线程数时，卡住的那一轮停在它开始后几秒、之后再无状态；
  加 `-- --test-threads=1` 的那一轮两个 job 全绿，随后推送到 main 的那一轮也全绿
  （`Native Win7+ Build` 与 `Setup End to End` 都 success）。
- 构建机卡死的定位走到了一个明确的结论（用 `suite_command` 派发口在**能复现的那台机器**上按模块二分，
  每轮约五分钟）：把 `install::`、`script::`、`service::`、`msi::` 四个模块一起跳掉那一轮是绿的（28 秒），
  只跳 `msi::`、只跳 `service`、只跳 `install::` 时都仍然卡住，而**只跑 `script::`** 也卡住——触发点在
  这 42 条用例里。继续往下切：`script::tests::a_script_*`（11 条）单独跑是绿的，另外两组候选
  （两条 cancel 用例；`a_failing_install_script_removes_what_it_wrote` 与
  `a_file_type_that_would_write_outside_the_classes_tree_is_refused`）单独跑也都是绿的，但把
  `script::tests::a_*`（15 条，也就是上面三组之和）一起跑就卡住。**所以没有哪一条用例是触发器**：
  它需要这些「真的跑一次安装」的用例**同时**在机器上跑，而 agent 只有两个 vCPU 且这一步是提权的。
  本机量过核心库这条命令的峰值内存只有 112 MiB（4 线程也一样），所以不是内存；本机是普通权限运行，
  走不到提权才有的那些分支，因此复现不出来。下一步要查的是这个「同时」到底撞上了什么（产品里
  `BUSY`/`TASK` 与安装日志都是进程级的单例，而测试是并行线程共享同一个进程），眼下先按
  `-- --test-threads=1` 处置：它让这些用例轮流跑，四轮 CI 与整套本机运行都是绿的。
- 发布门禁与摘要清单都试过真的：对 `a7a227a`（最新 push 运行 success）门禁放行，对 `f5b0aa2`
  （最新 push 运行 cancelled）门禁拒绝并说明原因；摘要脚本对一个替身的发布目录写出 5 行并逐行核对通过。
...
- `cargo test --locked --workspace --no-fail-fast`：355 条用例全部跑到，354 通过、0 失败、1 忽略
  （核心库 261 含 1 忽略、安装包级 58 都在真机上装了一遍、工程检查 5、可视化构建器 29、解压运行时 2）；
  ...
  `audit_case_descriptions.ps1` 报 `355 of 355 case(s) described`。
- 三个端到端用例与十二条单元用例把上面这些固定下来：包的身份与自定义动作、安装包本体作为流、
...
- `cargo test --locked --workspace --no-fail-fast`：共 338 条用例，337 通过、0 失败、1 忽略，退出码 0
  ...
  `target/test-report.txt` 与 `target/test-report.html`。
- 安装包级的 55 条全部跑通，其中 17 条会打开真实的向导窗口；`run_e2e_setup.ps1 -RequireDesktop` 把
  跳过当成失败，报告在 `target/e2e-report.txt` 与 `target/e2e-report.html`。
- 高对比这一轮的两层证据都在。核心库那七条给每个角色各配一个颜色：页面与卡片的填充和描边、页面上的字
  与按钮上的字、环、展开菜单里键盘所在的一行与当前选中的一行、滚动条轨道与滑块各取自己那一色，图片
  不被换色，系统没要求时逐像素回到声明值。安装包级那条把两种答案各跑一遍，夹具把机器自己的颜色对调着
  声明，于是帧上读到的是哪一套一看便知：关掉时是声明的那些，打开时是 `COLOR_WINDOW`、`COLOR_BTNFACE`
  与 `COLOR_WINDOWFRAME`，按一次 Tab 之后环是 `COLOR_HIGHLIGHT`，复选框那张图两种情形下都没变；最后
  发一条 `WM_SETTINGCHANGE`，逐像素对比前后两帧，窗口一个像素都没动、也还在。开关本身没有被用例动过：
  它属于坐在机器前的人，用例只把答案写进这次运行的进程。
...
- 提交 2cc7fbd 推上分支后 CI 两个作业全绿（运行 35727299196，5 分 44 秒）：`Setup End to End` 54 条全过
  ——新增的键盘用例和本机因锁屏跑不通的那两条指针用例都在里面（73.67 秒）；`Native Win7+ Build` 的工作区那一跑
  329 通过、0 失败、1 忽略（那个作业没有 stub，安装包级 54 条按跳过算通过），
  五个审计、fmt 与 clippy 全 exit 0。同一次推送触发的 PR 运行里 `Setup End to End` 也是绿的，
  `Native Win7+ Build` 那个作业卡在 `Test workspace`（这个作业两三次里卡一次，取消后重派即可）。
