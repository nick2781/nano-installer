...
- 每次测试运行都留下可以读的报告：一份文本和一份可以点开看的网页，列出每条用例、它的结果，以及
  ...
  不会被读成通过。
- 流水线的测试步骤不再挂住。它曾多次跑上几十分钟也不结束，连日志都留不下：步骤留下的进程继承着
  步骤自己的输出，而步骤要等输出关闭才算结束。现在命令的输出交给操作系统打开的普通文件，脚本读
  到的东西不再被构建的子孙进程握住；步骤自己启动的进程又都进了一个随脚本结束的作业，脚本退出时
  被留下的进程一并结束，输出随之关闭。命令还带上了期限：超时的命令会先把它最后写下的几十行打出来，
  再把命令那棵进程树整棵结束掉，步骤随后以失败结束，日志因此留在产物里，也能看出它停在了哪里。
  期限不再指望那个作业一定生效（有宿主不允许第二个作业），也不再有哪个命令是没有期限的：
  连 `cargo --version`、`git status` 这样的探测都各自带着两分钟，卡住的构建代理因此不会让整步
  无声无息地停在那里。
